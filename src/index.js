...
function generateDog(data){
    ...
    isGoodDog.addEventListener('click', () => {
        if(isGoodDog.textContent === 'Bad Dog!'){
            isGoodDog.textContent = 'Good Dog!'
            fetch(`http://localhost:3000/pups/${data.id}`, {method: "PATCH", headers: {"Content-type": "application/json"}, body: JSON.stringify({ isGoodDog: true})})
            .then(response => {console.log(response.status); return response.json(); })
            .then(data => console.log(data))
        }
        else if(isGoodDog.textContent === 'Good Dog!'){
            isGoodDog.textContent = 'Bad Dog!';
            fetch(`http://localhost:3000/pups/${data.id}`, {method: "PATCH", headers: {"Content-type": "application/json"}, body: JSON.stringify({ isGoodDog: false})})
            .then(response => {console.log(response.status); return response.json(); })
            .then(data => console.log(data))
        }
    })
